...
#include "nav2_regionalplanner/planner_server.hpp"
...
PlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  planlist_publisher_ = create_publisher<geometry_msgs::msg::PoseArray>("planlist", 1);

  // Create the action server that we implement with our navigateToPose method
  action_server_ = std::make_unique<ActionServer>(
    rclcpp_node_,
    "compute_regional_path",
...
PlannerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  planlist_publisher_->on_activate();
...
PlannerServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  planlist_publisher_->on_deactivate();
...
PlannerServer::computePlan()
{
  ...
  try {
    ...
    result->path_list = getPlan(goal->boundary, goal->planner_id);
    ...
    RCLCPP_DEBUG(
      get_logger(),
      "Found valid paths of size %u",
      result->path_list.size());

    // Publish the plan for visualization purposes
    publishPlanList(result->path_list);
...
{
  RCLCPP_DEBUG(
    get_logger(), "Attempting to paths in boundary(size:%lu), using planner [%s] .",
    boundary.poses.size(), planner_id.c_str());

  if (planners_.find(planner_id) != planners_.end()) {
    return planners_[planner_id]->createPlan(boundary);
  } else {
    if (planners_.size() == 1 && planner_id.empty()) {
      ...
      return planners_[planners_.begin()->first]->createPlan(boundary);
    } else {
      RCLCPP_ERROR(
  ...
  std::vector<nav_msgs::msg::Path> path_list;
  return path_list;
}

void
PlannerServer::publishPlanList(const std::vector<nav_msgs::msg::Path> & path_list)
{
  auto msg = std::make_unique<geometry_msgs::msg::PoseArray>();
  msg->header = path_list.front().header;
  ...
  if (
    planlist_publisher_->is_activated() &&
    this->count_subscribers(planlist_publisher_->get_topic_name()) > 0)
  {
    planlist_publisher_->publish(std::move(msg));
  }
}
