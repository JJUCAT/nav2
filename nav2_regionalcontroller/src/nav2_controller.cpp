...
#include "nav2_regionalcontroller/nav2_controller.hpp"

using namespace std::chrono_literals;

namespace nav2_regionalcontroller
{
...
: LifecycleNode("controller_server", "", true),
  ...
  default_progress_checker_type_{"nav2_regionalcontroller::SimpleProgressChecker"},
  goal_checker_loader_("nav2_core", "nav2_core::GoalChecker"),
  default_goal_checker_id_{"goal_checker"},
  default_goal_checker_type_{"nav2_regionalcontroller::SimpleGoalChecker"},
  lp_loader_("nav2_core", "nav2_core::Controller"),
  default_ids_{"RegionalFollowPath"},
  ...
  std::string & current_controller)
{
  if (controllers_.find(c_name) == controllers_.end()) {
    if (controllers_.size() == 1 && c_name.empty()) {
      ...
      RCLCPP_ERROR(
        get_logger(), "RegionalFollowPath called with controller name %s, "
